...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  try {
    return await requirePermission("expenseReports.manage", async (user) => {
      ...
      const updated = await expenseReportService.reimburse(
        {
          id,
      ...
      await auditLogService.recordFromRequest(request, {
        ...
        actorUserId: user.id,
        summary: `Masraf bildirimi ödendi: ${updated.reportNumber}`,
        metadata: { expenseReportId: updated.id, reimbursedAt: updated.reimbursedAt },
      });
