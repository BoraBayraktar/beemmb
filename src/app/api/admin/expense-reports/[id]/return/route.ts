...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  try {
    return await requirePermission("expenseReports.approve", async (user) => {
      ...
      const updated = await expenseReportService.return({ id, decisionNote: payload.decisionNote }, { id: user.id, tenantId: user.tenantId, hasManage });
      ...
      await auditLogService.recordFromRequest(request, {
        ...
        actorUserId: user.id,
        summary: `Masraf bildirimi düzenlenmesi için geri gönderildi: ${updated.reportNumber}`,
