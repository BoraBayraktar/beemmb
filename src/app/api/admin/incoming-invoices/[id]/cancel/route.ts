...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    const updated = await incomingInvoiceService.cancelIncomingInvoice(id, user.id);
    ...
    await auditLogService.recordFromRequest(request, {
      ...
      actorUserId: user.id,
      summary: `Gelen fatura iptal edildi: ${updated.documentNumber}`,
      metadata: { incomingInvoiceId: updated.id, status: updated.status },
    });
